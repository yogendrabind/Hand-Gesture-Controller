...
import cv2 # computer vision library to read video and pictures from camera.
import mediapipe as mp#cross-platform Machine Learning framework used for building complex and multimodal applied machine learning pipelines.
import pyautogui# Python pakage to simulate mouse cursor moves and keyboard input.
import math #for mathematical calculation
from enum import IntEnum #insure that members must be integer
from ctypes import cast, POINTER #for impoting and casting mouse pointer
from comtypes import CLSCTX_ALL #used in activation calls and response output
from pycaw.pycaw import AudioUtilities, IAudioEndpointVolume #python Core Audio Windows Library
from google.protobuf.json_format import MessageToDict# to take text as input
import screen_brightness_control as sbcontrol #to control brightness 

#initially Nothing has happen, to set False value.
pyautogui.FAILSAFE = False
# Initializing the drawng utils for drawing the facial landmarks on image
mp_drawing = mp.solutions.drawing_utils #To landmark every Points in hand and make a frame
mp_hands = mp.solutions.hands #To focus on hand only in video

# Gesture Encodings 
class Gest(IntEnum):#takin integer landmarks an input parameter
    # Binary Encoded
    """
    ...
    # Extra Mappings for hand picking
    V_GEST = 33
...
class HandRecog:
    ...
    def get_dz(self,point):
        ...
        float
        """
        return abs(self.hand_result.landmark[point[0]].z - self.hand_result.landmark[point[1]].z)#Eclide distance for Z axis
...
class Controller:
    ...
    def handle_controls(gesture, hand_result):  
        ...
        # flag reset to exwcute new gesture
...
# start hand gesture fuction and start function
#c1 = GestureController()
#gc1.start()